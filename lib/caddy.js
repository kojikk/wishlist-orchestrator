...
/** Результат последнего синка — отдаётся в админку (GET /api/admin/caddy). */
let lastSync = null;
...
/**
 * Полный цикл: build → push → лог. Результат запоминается для админки.
 */
async function syncCaddy(registry) {
  if (!CADDY_ADMIN_URL) {
    console.log('[caddy] CADDY_ADMIN_URL not set — sync skipped');
    lastSync = { at: Date.now(), ok: false, skipped: true, reason: 'CADDY_ADMIN_URL not set' };
    return lastSync;
  }
  ...
  lastSync = {
    ...
    error: result.error || (result.ok ? null : result.body || result.reason || null),
    hosts,
  };
  if (result.ok) {
...
module.exports = { buildCaddyfile, pushToCaddy, syncCaddy, getLastSync };
