...
class Registry {
  constructor({ configPath, refreshIntervalMs }) {
    ...
    /** Хост самого оркестратора (для Caddy-генератора). */
    this.selfHost = null;
    /** Внутренний адрес оркестратора в docker-сети (для Caddy upstream). */
    this.selfUpstream = null;
    /** Колбэк, вызываемый после loadConfig — используется для синка Caddy. */
  ...
  loadConfig() {
    ...
    this.selfHost     = cfg.self?.host     || null;
    this.selfUpstream = cfg.self?.upstream || null;
    ...
    if (typeof this.onConfigChange === 'function') {
      try { this.onConfigChange(this); } catch (e) { console.warn('[registry] onConfigChange:', e.message); }
    }
  }
